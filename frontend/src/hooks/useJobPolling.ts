...
import type { ContentGenerationResponse, JobStatus } from '../api/types';
...
// Estimated step durations for progress calculation (in seconds)
...
export function useJobPolling(
  ...
  const poll = useCallback(async () => {
    ...
    try {
      const response = await geoApi.getJobStatus(jobId);
      ...
      if (attemptsRef.current >= maxAttempts) {
        setError('Polling timeout - job took too long');
        isPollingActiveRef.current = false;
        setIsPolling(false);
